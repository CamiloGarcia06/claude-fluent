# claude-fluent — diseño y criterios

El documento original de diseño (antes el `CLAUDE.md`), conservado íntegro:
es la explicación larga de por qué el app es como es. Las reglas que impone
viven ahora en el código (dominio de cada funcionalidad) y en `docs/adr/`.
Las rutas de módulos que menciona son las anteriores a la migración al
estándar hexagonal (septiembre de 2026); el mapa actual está en `README.md`.
