...
def propose(generate, client, note: dict) -> dict:
    ...
    current = {n: to_plain_text(v) for n, v in client.note_fields(note).items()}

    result, duration_ms = generate(
        PROMPT.format(
            model=note["modelName"],
            fields=_fields_block(current),
            history=_history_block(client, note),
        ),
        REPAIR_SCHEMA,
    )
    ...
    for item in result.get("fields", []):
        name = item.get("name", "")
        if name in current:
            proposed[name] = to_plain_text(str(item.get("value", "")))
        elif name:
