...
# Nombre histórico; los adaptadores nuevos capturan Upstream o ModelError.
LLMError = ModelError
...
class ClaudeCli:
    """El modelo como objeto inyectable: `generate(prompt, schema, system=...)`."""

    def generate(
        self, prompt: str, schema: dict, timeout: int = TIMEOUT_S, system: str = SYSTEM_PROMPT
    ) -> tuple[dict, int]:
        return generate(prompt, schema, timeout=timeout, system=system)
