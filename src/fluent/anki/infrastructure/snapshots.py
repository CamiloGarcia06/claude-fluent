...
from fluent.anki.infrastructure import client as anki
...
class Snapshots:
    """El camino de escritura como objeto, para inyectarlo (ver AnkiClient)."""

    def capture(self, note_id: int) -> Path:
        return capture(note_id)

    def update_note_fields(self, note_id: int, fields: dict[str, str]) -> Path:
        return update_note_fields(note_id, fields)

    def add_notes(self, deck: str, notes: list[dict]) -> tuple[list[int], Path, list[dict]]:
        return add_notes(deck, notes)

    def ensure_model(
        self, name: str, fields: list[str], templates: list[dict], css: str
    ) -> Path | None:
        return ensure_model(name, fields, templates, css)

    def undo_creation(self, record_path: Path | str) -> int:
        return undo_creation(record_path)

    def move_cards(self, card_ids: list[int], target: str) -> Path:
        return move_cards(card_ids, target)

    def undo_move(self, record_path: Path | str) -> int:
        return undo_move(record_path)

    def delete_empty_deck(self, name: str) -> bool:
        return delete_empty_deck(name)

    def restore(self, snapshot_path: Path | str) -> Path:
        return restore(snapshot_path)

    def history(self, note_id: int) -> list[dict]:
        return history(note_id)
